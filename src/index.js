...
app.post('/api/users/login', async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length > 0) {
      res.json({ user_id: result.rows[0].user_id, username });
    } else {
...
app.post('/api/transactions', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (type === 'BUY') {
        const usdWallet = await client.query('SELECT balance FROM wallets WHERE user_id = $1 AND coin_id = 0', [user_id]);
        if (usdWallet.rows[0].balance < total) throw new Error('餘額不足');
        await client.query('UPDATE wallets SET balance = balance - $1 WHERE user_id = $2 AND coin_id = 0', [total, user_id]);
        await client.query(
        ...
        await client.query(
          'INSERT INTO wallets (user_id, coin_id, balance) VALUES ($1, 0, $2) ON CONFLICT (user_id, coin_id) DO UPDATE SET balance = wallets.balance + $2',
          [user_id, total]
        );
      }
      await client.query(
